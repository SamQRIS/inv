            ...
            ->recordActions([
                ...
                ActionGroup::make([
                    Action::make('mutasi_stok')
                        ...
                        ->color('warning')
                        ->form(function (\Illuminate\Database\Eloquent\Model $record) {
                            /** @var Warehouse $record */
                            $warehouseId = $record->id;

                            // ✅ Tampilkan stok langsung di option label — tanpa live()
                            $productOptions = \App\Models\ProductStock::where('warehouse_id', $warehouseId)
                                ->with('product.unit')
                                ->get()
                                ->mapWithKeys(fn($ps) => [
                                    $ps->product_id => "{$ps->product->name} (Stok: {$ps->quantity} {$ps->product->unit->symbol})"
                                ])
                                ->union(
                                    // Produk aktif yang belum punya stok di gudang ini
                                    Product::active()
                                        ->whereNotIn('id', \App\Models\ProductStock::where('warehouse_id', $warehouseId)->pluck('product_id'))
                                        ->get()
                                        ->mapWithKeys(fn($p) => [$p->id => "{$p->name} (Stok: 0)"])
                                );
                            ...
                            return [
                                Select::make('product_id')
                                    ...
                                    ->searchable()
                                    ->required(),
                                // ❌ Hapus Placeholder current_stock — info sudah ada di option label
                        ...
                        ->action(function (\Illuminate\Database\Eloquent\Model $record, array $data) {
                            ...
                            match ($data['type']) {
                                'in'         => $stockService->addStock($product, (int)$data['quantity'], $record, 'manual', null, $data['notes']),
                                'out'        => $stockService->reduceStock($product, (int)$data['quantity'], $record, 'manual', 0, $data['notes']),
                                'adjustment' => $stockService->adjustStock($product, $record, (int)$data['quantity'], $data['notes']),
                            };

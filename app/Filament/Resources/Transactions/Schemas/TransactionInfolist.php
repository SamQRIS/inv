                    ...
                    ->schema([
                        RepeatableEntry::make('items')->label('')
                            ->schema([
                                ...
                                // Di kolom items repeater
                                TextEntry::make('qty_backorder')
                                    ->label('Backorder')
                                    ->visible(fn($record) => $record->is_backorder)
                                    ->color('warning')
                                    ->formatStateUsing(fn($state) => $state > 0 ? "⚠️ {$state} unit perlu diorder" : null),
                            ])
                            ->columns(4),
                    ]),
